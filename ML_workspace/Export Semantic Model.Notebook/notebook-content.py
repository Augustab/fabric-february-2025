...
for idx, row in semantic_model_tables.iterrows():
    ...
    try:
        ...
        #df = spark.sql("SELECT * FROM pbi.`{semantic_model}`.`{table}` GROUP BY <col> HAVING <expr> WHERE <expr>")

        dst_path = labs.create_abfss_path(lakehouse_id, workspace_id, f"{table}_manually")
